#!/usr/bin/env python3
"""
股票分析后端API服务器
提供每日、每周、资金流入、板块联动、AI预测、选股建议等API接口
"""

from flask import Flask, jsonify
import json
import datetime

app = Flask(__name__)

# 示例数据 - 后续可替换为真实数据
SAMPLE_DATA = {
    "daily_analysis": [
        {
            "sector": "科技板块",
            "change_percent": 3.5,
            "volume": 10000000,
            "ranking": 1
        },
        {
            "sector": "金融板块",
            "change_percent": 2.1,
            "volume": 8000000,
            "ranking": 2
        },
        {
            "sector": "医疗板块",
            "change_percent": -1.2,
            "volume": 5000000,
            "ranking": 3
        },
        {
            "sector": "新能源板块",
            "change_percent": 4.8,
            "volume": 7000000,
            "ranking": 4
        },
        {
            "sector": "消费板块",
            "change_percent": 0.5,
            "volume": 6000000,
            "ranking": 5
        }
    ],
    "weekly_analysis": [
        {
            "sector": "科技板块",
            "weekly_change": 8.2,
            "avg_volume": 12000000,
            "trend": "上升"
        },
        {
            "sector": "金融板块",
            "weekly_change": 4.5,
            "avg_volume": 9000000,
            "trend": "平稳"
        },
        {
            "sector": "医疗板块",
            "weekly_change": -3.1,
            "avg_volume": 5500000,
            "trend": "下降"
        },
        {
            "sector": "新能源板块",
            "weekly_change": 12.8,
            "avg_volume": 8500000,
            "trend": "上升"
        },
        {
            "sector": "消费板块",
            "weekly_change": 1.2,
            "avg_volume": 7000000,
            "trend": "平稳"
        }
    ],
    "capital_flow": [
        {
            "sector": "科技板块",
            "capital_in": 150000000,
            "capital_out": 50000000,
            "net_inflow": 100000000
        },
        {
            "sector": "金融板块",
            "capital_in": 80000000,
            "capital_out": 40000000,
            "net_inflow": 40000000
        },
        {
            "sector": "医疗板块",
            "capital_in": 60000000,
            "capital_out": 80000000,
            "net_inflow": -20000000
        },
        {
            "sector": "新能源板块",
            "capital_in": 120000000,
            "capital_out": 30000000,
            "net_inflow": 90000000
        },
        {
            "sector": "消费板块",
            "capital_in": 70000000,
            "capital_out": 60000000,
            "net_inflow": 10000000
        }
    ],
    "sector_linkage": {
        "nodes": [
            {"id": "科技", "name": "科技板块", "group": 1},
            {"id": "新能源", "name": "新能源板块", "group": 2},
            {"id": "金融", "name": "金融板块", "group": 3},
            {"id": "医疗", "name": "医疗板块", "group": 4},
            {"id": "消费", "name": "消费板块", "group": 5}
        ],
        "links": [
            {"source": "科技", "target": "新能源", "value": 0.8},
            {"source": "科技", "target": "金融", "value": 0.6},
            {"source": "新能源", "target": "医疗", "value": 0.4},
            {"source": "金融", "target": "消费", "value": 0.7},
            {"source": "医疗", "target": "科技", "value": 0.3}
        ]
    },
    "predictions": [
        {
            "sector": "科技板块",
            "predicted_change": 2.5,
            "confidence": 85,
            "recommendation": "买入"
        },
        {
            "sector": "金融板块",
            "predicted_change": 1.8,
            "confidence": 70,
            "recommendation": "持有"
        },
        {
            "sector": "医疗板块",
            "predicted_change": -0.5,
            "confidence": 60,
            "recommendation": "观望"
        },
        {
            "sector": "新能源板块",
            "predicted_change": 4.2,
            "confidence": 90,
            "recommendation": "强烈买入"
        },
        {
            "sector": "消费板块",
            "predicted_change": 0.9,
            "confidence": 65,
            "recommendation": "持有"
        }
    ],
    "stock_suggestions": [
        {
            "name": "腾讯控股",
            "code": "00700",
            "sector": "科技板块",
            "price": 350.2,
            "change_percent": 2.3,
            "score": 87,
            "reason": "龙头地位稳固，业务多元化"
        },
        {
            "name": "宁德时代",
            "code": "300750",
            "sector": "新能源板块",
            "price": 180.5,
            "change_percent": 5.8,
            "score": 92,
            "reason": "新能源龙头，技术领先"
        },
        {
            "name": "招商银行",
            "code": "600036",
            "sector": "金融板块",
            "price": 32.8,
            "change_percent": 1.2,
            "score": 75,
            "reason": "稳健经营，盈利能力强"
        },
        {
            "name": "迈瑞医疗",
            "code": "300760",
            "sector": "医疗板块",
            "price": 280.0,
            "change_percent": -0.8,
            "score": 68,
            "reason": "医疗设备龙头，短期波动"
        },
        {
            "name": "贵州茅台",
            "code": "600519",
            "sector": "消费板块",
            "price": 1500.0,
            "change_percent": 0.5,
            "score": 82,
            "reason": "消费品龙头，品牌价值高"
        }
    ]
}

@app.route('/api/stock/daily', methods=['GET'])
def daily_analysis():
    """每日板块分析"""
    return jsonify({
        "data": SAMPLE_DATA["daily_analysis"],
        "timestamp": datetime.datetime.now().isoformat()
    })

@app.route('/api/stock/weekly', methods=['GET'])
def weekly_analysis():
    """每周板块分析"""
    return jsonify({
        "data": SAMPLE_DATA["weekly_analysis"],
        "timestamp": datetime.datetime.now().isoformat()
    })

@app.route('/api/stock/capital', methods=['GET'])
def capital_flow():
    """资金流入流出分析"""
    return jsonify({
        "data": SAMPLE_DATA["capital_flow"],
        "timestamp": datetime.datetime.now().isoformat()
    })

@app.route('/api/stock/sectors', methods=['GET'])
def sector_linkage():
    """板块联动分析"""
    return jsonify({
        "data": SAMPLE_DATA["sector_linkage"],
        "timestamp": datetime.datetime.now().isoformat()
    })

@app.route('/api/stock/predictions', methods=['GET'])
def predictions():
    """AI预测和建议"""
    return jsonify({
        "data": SAMPLE_DATA["predictions"],
        "timestamp": datetime.datetime.now().isoformat()
    })

@app.route('/api/stock/suggestions', methods=['GET'])
def stock_suggestions():
    """选股建议"""
    return jsonify({
        "data": SAMPLE_DATA["stock_suggestions"],
        "timestamp": datetime.datetime.now().isoformat()
    })

@app.route('/api/health', methods=['GET'])
def health_check():
    """健康检查"""
    return jsonify({
        "status": "healthy",
        "version": "1.0.0",
        "timestamp": datetime.datetime.now().isoformat()
    })

if __name__ == '__main__':
    app.run(host='0.0.0.0', port=5000, debug=True)